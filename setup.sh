...
# Idempotent: if config already exists and contains a USERNAME, reuse it.
if [ -f "$CONFIG" ]; then
    EXISTING_USERNAME=$(grep -E "^USERNAME=" "$CONFIG" | head -1 | cut -d= -f2 | tr -d '"'"' '"'"')
fi

if [ -n "${EXISTING_USERNAME:-}" ]; then
    USERNAME="$EXISTING_USERNAME"
    echo "  Existing config found — username: $USERNAME (skipping prompt)"
    echo "  (To change, edit $CONFIG or delete it and re-run setup.sh)"
else
...
# ── Step 6: Write config and initialize database ─────────────────────────────
# Config must be written BEFORE setup_db.py runs so it can read the username
# and seed the correct values into the users and agents tables.
echo "[6/8] Writing config and initializing database..."

# CONFIG was set in Step 3 (idempotent path uses same variable)
if [ -f "$CONFIG" ]; then
    echo "  Config already exists at $CONFIG — keeping existing settings."
else
    cat > "$CONFIG" << CONF
...
# ── Step 8: Generate session prompt ─────────────────────────────────────────
echo "[8/8] Generating session prompt..."
